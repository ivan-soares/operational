...
#############################################################################################################
	...
	echo
	echo " +++ Starting bash program run_shel on grid $grds +++"
	echo
...
##### Code compilation options
	...
	if [ $icomp == 'yes' ]; then
	     ...
	     ${path_b}/w3_make ww3_grid ww3_strt ww3_prnc ww3_bounc
	     if [ "$mpi" = "yes" ]; then
...
#######################################################################################
        ...
        echo ; echo " ... run ww3_bounc"; echo

        rm -f mod_def.ww3
        ln -s mod_def.$grds mod_def.ww3
        cp $path_i/ww3_bounc.inp .

        echo " ... model output directed to $path_r/logfile_bounc.log"
        echo

        $path_e/ww3_bounc > logfile_bounc.log
        rm -f mod_def.ww3 ww3_bounc.inp

	echo ; echo " ... finished ww3_bounc"; echo

#######################################################################################
#                                                                                     #
#                           5. MAIN PROGRAM: WW3_SHEL                                 #
#                                                                                     #
#######################################################################################
	...
	echo " ... run shel "
	echo 
	echo " ... model output directed to $path_r/logfile_shel.log"
	echo

	rm -f mod_def.ww3
        ln -s mod_def.$grds mod_def.ww3
	cp $path_i/ww3_shel.inp .


	if [ "$mpi" = 'yes' ]; then
	   mpirun -np $nprocs $path_e/ww3_shel >& logfile_shel.log &
	   #srun --mpi=pmi2 -n $nprocs $path_e/ww3_multi >& logfile_multi.log &
	   wait
	else
	   $path_e/ww3_shel  >& logfile_shel.log &
	   wait
	fi

	rm ww3_shel.inp

	echo ; echo " ... finished ww3_shel"; echo
...
#######################################################################################
	...
	mv out_grd.ww3     ww3_out_grd_${t_ini}.$grds
	mv restart001.ww3  ww3_out_rst_${t_rst}.$grds

	if [ $buoy == 'points' ]; then 
	    mv out_pnt.ww3  ww3_out_pnt_${t_ini}.points
	fi

	cd $case_dir

	echo
	echo " +++ End of bash program run_ww3_shel +++"
	echo
