...
#
       ...
       rm -rf dstart
       python $pythdir/find_dstart.py $today
...
###################### *** fix the ocean.in file *** #####################################
       ...
       sed -e "s|INPUTS|$roms_inpdir|g" -e "s|OUTPUTS|$roms_outdir|g" \
           $roms_codedir/roms_brz0.05r.in >& ocean_${expt}.in
       ...
       ### grid size
       ### Lm is Number of I-direction INTERIOR RHO-points
       ### Mm is Number of J-direction INTERIOR RHO-points
      
       let lm=$nx-2
       let mm=$ny-2

       sed -i "/ Lm == /    c\  Lm == $lm "      ocean_${expt}.in
       sed -i "/ Mm == /    c\  Mm == $mm "      ocean_${expt}.in

       ### mpi tiles
       ### NtileI is I-direction partition
       ### NtileJ is J direction partition
       ...
       sed -i "/ NQCK == /    c\  NQCK == $nhis "         ocean_${expt}.in
       ...
       sed -i "/ LwSrc == /      c\ LwSrc == F         ! volume vertical influx "        ocean_${expt}.in
       sed -i "/ LuvSrc == /     c\ LuvSrc ==  F       ! horizontal momentum transport " ocean_${expt}.in
       sed -i "/ LtracerSrc == / c\ LtracerSrc == F F  ! temperature, salinity, inert  " ocean_${expt}.in
       ...
       sed -i " / LBC(isUvel) == /c\ LBC(isUvel) ==  Clo     RadNud  RadNud  RadNud     ! 3D U-momentum"  ocean_${expt}.in
       sed -i " / LBC(isVvel) == /c\ LBC(isVvel) ==  Clo     RadNud  RadNud  RadNud     ! 3D V-momentum"  ocean_${expt}.in
       ...
       sed -i "/ LsshCLM == / c\ LsshCLM == F "  ocean_${expt}.in
       sed -i "/ Lm2CLM == /  c\  Lm2CLM == F "  ocean_${expt}.in
       sed -i "/ Lm3CLM == /  c\  Lm3CLM == T "  ocean_${expt}.in
       sed -i "/ LtracerCLM == / c\ LtracerCLM == T T " ocean_${expt}.in

       sed -i "/ LnudgeM2CLM == / c\ LnudgeM2CLM == F " ocean_${expt}.in
