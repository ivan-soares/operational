...
user=`pwd | sed 's/\// /g' | awk '{print $2}'`

echo 
echo " ... Running script for user $HOME"
echo
...
#       export PATH=/opt/intelsoft/openmpi/bin:$PATH
    ...
    gfortran )
      if [ "${which_MPI}" = "mpich" ]; then
        export PATH=$HOME/apps/mpich-3.2/bin:$PATH
      elif [ "${which_MPI}" = "openmpi" ]; then
        export PATH=$HOME/apps/openmpi-1.10.1/bin:$PATH
      fi
      ;;
...
if [ -n "${USE_MY_LIBS:+1}" ]; then
  case "$FORT" in
    ...
    gfortran )
      ...
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich" ]; then
          export        ESMF_DIR=$HOME/apps/mpich-3.2/esmf
          export      MCT_INCDIR=$HOME/apps/mpich-3.2/mct/include
          export      MCT_LIBDIR=$HOME/apps/mpich-3.2/mct/lib
          export  PARPACK_LIBDIR=$HOME/apps/mpich-3.2/PARPACK
        elif [ "${which_MPI}" = "openmpi" ]; then
          export        ESMF_DIR=$HOME/apps/openmpi-1.10.1/esmf
          export      MCT_INCDIR=$HOME/apps/openmpi-1.10.1/mct/include
          export      MCT_LIBDIR=$HOME/apps/openmpi-1.10.1/mct/lib
          export  PARPACK_LIBDIR=$HOME/apps/openmpi-1.10.1/PARPACK
        fi
      fi
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            export     NC_CONFIG=$HOME/apps/netcdf-4.7.1/bin/nc-config
	    export     NF_CONFIG=$HOME/apps/netcdf-4.7.1/bin/nf-config
            export NETCDF_INCDIR=$HOME/apps/netcdf-4.7.1/include
          elif [ "${which_MPI}" = "openmpi" ]; then
            export     NC_CONFIG=$HOME/apps/netcdf-4.7.1/bin/nc-config
	    export     NF_CONFIG=$HOME/apps/netcdf-4.7.1/bin/nf-config
            export NETCDF_INCDIR=$HOME/apps/netcdf-4.7.1/include
