...
echo
echo " ... Running Build script for user $HOME"
echo
...
export   ROMS_APPLICATION=BCAMPOS
  ...
  case "$FORT" in
    ...
    gfortran )
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            export     NC_CONFIG=$HOME/apps/netcdf-4.4.all/bin/nc-config
            export NETCDF_INCDIR=$HOME/apps/netcdf-4.4.all/include
          elif [ "${which_MPI}" = "openmpi" ]; then
            export     NC_CONFIG=$HOME/apps/netcdf-4.4.all/bin/nc-config
            export NETCDF_INCDIR=$HOME/apps/netcdf-4.4.all/include
          fi
