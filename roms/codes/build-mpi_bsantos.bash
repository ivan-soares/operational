...
export   ROMS_APPLICATION=BSANTOS
...
#       export PATH=/opt/intelsoft/openmpi/bin:$PATH
    ...
    gfortran )
      if [ "${which_MPI}" = "mpich" ]; then
        ...
        export PATH=$HOME/apps/openmpi-4.1.1/bin:$PATH
      fi
      ;;
    ...
    gfortran )
      ...
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich" ]; then
          ...
          export        ESMF_DIR=$HOME/apps/openmpi-4.1.1/esmf
          export      MCT_INCDIR=$HOME/apps/openmpi-4.1.1/mct/include
          export      MCT_LIBDIR=$HOME/apps/openmpi-4.1.1/mct/lib
          export  PARPACK_LIBDIR=$HOME/apps/openmpi-4.1.1/PARPACK
        fi
      fi
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            export     NC_CONFIG=$HOME/apps/netcdf-c-4.8.0/bin/nc-config
            export NETCDF_INCDIR=$HOME/apps/netcdf-c-4.8.0/include
          elif [ "${which_MPI}" = "openmpi" ]; then
            export     NC_CONFIG=$HOME/apps/netcdf-c-4.8.0/bin/nc-config
            export NETCDF_INCDIR=$HOME/apps/netcdf-c-4.8.0/include
          fi
