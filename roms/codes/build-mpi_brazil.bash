...
export   ROMS_APPLICATION=BRAZIL_MY25

echo
echo " +++ Running build-mpi for ROMS application $ROMS_APPLICATION +++"
echo
  ...
  case "$FORT" in
    ifort )
      if [ "${which_MPI}" = "mpich" ]; then
        export PATH=/data1/local/bin:$PATH
...
#       export PATH=/opt/intelsoft/openmpi/bin:$PATH
        export PATH=/data1/local/openmpi/bin:$PATH
      fi
      ;;
    ...
    gfortran )
      if [ "${which_MPI}" = "mpich" ]; then
        export PATH=/home/ivans/apps/mpich-3.2/bin:$PATH
      elif [ "${which_MPI}" = "openmpi" ]; then
        export PATH=/home/ivans/apps/openmpi-1.10.1/bin:$PATH
      fi
      ;;
...
if [ -n "${USE_MY_LIBS:+1}" ]; then
  case "$FORT" in
    ifort )
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            export     NC_CONFIG=/data1/local/bin/nc-config
            export NETCDF_INCDIR=/data1/local/include
          elif [ "${which_MPI}" = "mpich2" ]; then
            export     NC_CONFIG=/opt/netcdf/bin/nf-config
            export NETCDF_INCDIR=/opt/netcdf/include
          elif [ "${which_MPI}" = "openmpi" ]; then
            export     NC_CONFIG=/data1/local/bin/nc-config
            export NETCDF_INCDIR=/data1/local/include
          fi
          ...
          export       NC_CONFIG=/data1/local/bin/nc-config
          export       NF_CONFIG=/data1/local/bin/nf_config
          export   NETCDF_INCDIR=/data1/local/include
        fi
      else
        export     NETCDF_INCDIR=/home/ivans/apps/netcdf-3.6.3/include
        export     NETCDF_LIBDIR=/home/ivans/apps/netcdf-3.6.3/lib
      fi
      ;;
    ...
    gfortran )
      ...
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich" ]; then
          export        ESMF_DIR=/home/ivans/apps/mpich-3.2/esmf
          export      MCT_INCDIR=/home/ivans/apps/mpich-3.2/mct/include
          export      MCT_LIBDIR=/home/ivans/apps/mpich-3.2/mct/lib
          export  PARPACK_LIBDIR=/home/ivans/apps/mpich-3.2/PARPACK
        elif [ "${which_MPI}" = "openmpi" ]; then
          export        ESMF_DIR=/home/ivans/apps/openmpi-4.1.1/esmf
          export      MCT_INCDIR=/home/ivans/apps/openmpi-4.1.1/mct/include
          export      MCT_LIBDIR=/home/ivans/apps/openmpi-4.1.1/mct/lib
          export  PARPACK_LIBDIR=/home/ivans/apps/openmpi-4.1.1/PARPACK
        fi
      fi
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            export     NC_CONFIG=/home/ivans/apps/netcdf-c-4.8.0/bin/nc-config
            export NETCDF_INCDIR=/home/ivans/apps/netcdf-c-4.8.0/include
          elif [ "${which_MPI}" = "openmpi" ]; then
            export     NC_CONFIG=/home/ivans/apps/netcdf-c-4.8.0/bin/nc-config
            export NETCDF_INCDIR=/home/ivans/apps/netcdf-c-4.8.0/include
          fi
        ...
        export     NETCDF_INCDIR=/home/ivans/apps/netcdf3/include
        export     NETCDF_LIBDIR=/home/ivans/apps/netcdf3/lib
      fi
      ;;
